(function(){
  ...
  function run(){
    if(contactId){
      ...
      xhr.open('GET','https://paymegpt.com/api/landing/context/'+encodeURIComponent(contactId)+'?page_id=2425');
      xhr.onload=function(){
        if(xhr.status===200){
          try{
...
(function(){
  ...
  window.__donationConfig={currency:currency,presets:[5,10,25,50],minAmount:100/100,maxAmount:1000000/100,formatAmount:fmtD,buttonColor:'#36D6B5',buttonText:'Support the Stream 👑',thankYouMessage:'You\'re officially CBB Nation royalty. Thank you! 👑🙏'};
  ...
  document.addEventListener('DOMContentLoaded',function(){
    ...
    if(urlParams.get('donation')==='success'){
      ...
      overlay.style.cssText='position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center;z-index:999999;font-family:system-ui,-apple-system,sans-serif;';
      overlay.innerHTML='<div style="background:white;border-radius:16px;padding:40px;max-width:420px;width:90%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.15);"><div style="width:64px;height:64px;border-radius:50%;background:#dcfce7;margin:0 auto 20px;display:flex;align-items:center;justify-content:center;"><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#16a34a" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg></div><h2 style="margin:0 0 12px;font-size:24px;font-weight:700;color:#111827;">Thank You!</h2><p style="margin:0 0 24px;color:#6b7280;font-size:16px;">You\'re officially CBB Nation royalty. Thank you! 👑🙏</p><button onclick="document.getElementById(\'donation-success-overlay\').remove();window.history.replaceState({},\'\',window.location.pathname);" style="padding:12px 32px;font-size:16px;font-weight:600;background:#16a34a;color:white;border:none;border-radius:8px;cursor:pointer;">Continue</button></div>';
      document.body.appendChild(overlay);
    }
  });
})();

const audio=document.getElementById('anthem'),unmuteBtn=document.getElementById('unmuteBtn'),musicToggle=document.getElementById('musicToggle'),muteToggle=document.getElementById('muteToggle');
    function sync(){const p=!audio.paused;musicToggle.textContent=p?'❚❚':'▶';muteToggle.textContent=audio.muted?'UNMUTE':'MUTE'}
    ['click','touchstart'].forEach(e=>document.addEventListener(e,()=>audio.play().catch(()=>{}),{once:true}));
    unmuteBtn.onclick=()=>{audio.muted=false;audio.play().catch(()=>{});sync()};
    musicToggle.onclick=()=>{audio.paused?audio.play():audio.pause();sync()};
    muteToggle.onclick=()=>{audio.muted=!audio.muted;if(!audio.paused)audio.play().catch(()=>{});sync()};
    audio.onplay=audio.onpause=audio.onvolumechange=sync;sync();

    const target=new Date('2026-03-22T22:00:00-05:00').getTime(),ids=['d','h','m','s'];
    setInterval(()=>{const n=Date.now(),t=Math.max(0,target-n),d=Math.floor(t/864e5),h=Math.floor(t%864e5/36e5),m=Math.floor(t%36e5/6e4),s=Math.floor(t%6e4/1e3);[d,h,m,s].forEach((v,i)=>document.getElementById(ids[i]).textContent=String(v).padStart(2,'0'));},1000);

    const cfg=(window.__donationConfig||{}),presets=(cfg.presets||[5,10,25,50]),wrap=document.getElementById('donationPresets'),status=document.getElementById('donationStatus');
    presets.forEach(v=>{const b=document.createElement('button');b.className='glass rounded-3xl p-6 text-left hover:border-[#36D6B5]/40 transition';b.innerHTML='<div class="text-[#36D6B5] tracking-[.3em] text-sm">DONATE</div><div class="mt-2 text-4xl font-black">$'+v+'</div><div class="mt-2 text-white/60">Send support to the stream</div>';b.onclick=()=>{status.textContent='Thank you for supporting the stream 👑';window.__processDonation(v*100)};wrap.appendChild(b)});
    document.getElementById('buyPlaybook').onclick=()=>{status.textContent='Playbook checkout initiated.';window.__processDonation(2900)};